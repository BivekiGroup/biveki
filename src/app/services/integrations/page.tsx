...
async function fetchCases() {
  const res = await fetch(`${getInternalApiBase()}/api/graphql`, {
    ...
    body: JSON.stringify({ query: `query{ cases(service:integrations, limit: 2){ slug title summary } }` }), cache: 'no-store'
  });
...
const ADVANTAGES = [
  "CRM/ERP интеграции (Bitrix24, 1С)",
  "Платёжные и уведомления (ЮKassa, CloudPayments, Telegram)",
  "Боты, webhooks, ETL‑синхронизации",
  "Логи, ретраи и мониторинг",
];

export default async function IntegrationsServicePage() {
    ...
    <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
      <section>
        ...
        <p className="mt-3 max-w-2xl text-sm leading-6 text-neutral-700 dark:text-neutral-300 sm:text-base">
          Соединяем системы, настраиваем обмен данными и автоматизируем рутину — прозрачно и надёжно.
        </p>
      </section>
      ...
      <section className="mt-8">
        ...
        <div className="mt-3 grid gap-3 lg:grid-cols-3">
          {[{
            ...
            price: "от 30 000 ₽",
            features: ["CRM/ERP", "Почта/уведомления", "Документация"],
          }, {
            name: "Вебхуки и платежи",
            price: "от 50 000 ₽",
            features: ["ЮKassa/CloudPayments", "Ретраи и логи", "Алерты"],
          }, {
            name: "Индивидуально",
            price: "от 80 000 ₽",
            features: ["ETL‑потоки", "Сложные схемы", "SLA"],
          }].map((p) => (
            <div key={p.name} className="relative overflow-hidden rounded-2xl border border-neutral-900/10 bg-white/70 p-5 dark:border-white/10 dark:bg-white/[0.06]">
              <div className="absolute inset-x-0 -top-px h-1 bg-gradient-to-r from-transparent via-emerald-500/40 to-transparent" />
              ...
              <ul className="mt-3 space-y-1 text-sm text-neutral-700 dark:text-neutral-300">
                {p.features.map((f) => (
                  <li key={f} className="flex items-start gap-2"><span className="mt-1 inline-block h-1.5 w-1.5 rounded-full bg-emerald-500" />{f}</li>
                ))}
              </ul>
              <div className="mt-4">
                <Link href="/contacts" className="inline-flex h-9 items-center rounded-full bg-gradient-to-r from-emerald-600 to-teal-500 px-4 text-sm font-semibold text-white shadow-sm hover:from-emerald-700 hover:to-teal-600">Запросить предложение</Link>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="mt-10">
        <div className="relative overflow-hidden rounded-2xl border border-neutral-900/10 bg-gradient-to-r from-emerald-600 to-teal-500 p-5 text-white shadow-sm dark:border-white/10">
          <div className="text-base font-semibold tracking-tight">Построим устойчивые интеграции</div>
          <p className="mt-1 text-sm opacity-90">Опишем контракты и ошибки, настроим ретраи и алерты.</p>
          <div className="mt-3 flex flex-wrap gap-2">
            <Link href="/contacts" className="inline-flex h-9 items-center rounded-full bg-white px-4 text-sm font-semibold text-emerald-700 shadow-sm hover:bg-white/90">
