...
async function fetchCases() {
  const res = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL || ''}/api/graphql`, {
    ...
    body: JSON.stringify({ query: `query{ cases(service:shop, limit: 2){ slug title summary } }` }), cache: 'no-store'
  });
...
const ADVANTAGES = [
  "Каталог, фильтры и вариации",
  "Корзина, оплатa (ЮKassa/CloudPayments)",
  "Доставка, статусы и уведомления",
  "Интеграции со складом и CRM",
];
    ...
    <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
      <section>
        ...
        <h1 className="mt-4 text-3xl font-semibold tracking-tight text-neutral-900 dark:text-neutral-50 sm:text-5xl sm:leading-tight">
          Интернет‑магазины и оплата
        </h1>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-neutral-700 dark:text-neutral-300 sm:text-base">
          Полный e‑commerce стек: каталог, корзина, эквайринг, доставка и интеграции со складом и CRM.
        </p>
      </section>
      ...
      <section className="mt-8">
        ...
        <div className="mt-3 grid gap-3 lg:grid-cols-3">
          {[{
            ...
            price: "55 000 ₽",
            features: ["Каталог", "Корзина", "Оплата"],
          }, {
            name: "Магазин+",
            price: "от 75 000 ₽",
            features: ["Доставка", "ЛК покупателя", "Интеграции"],
          }, {
            name: "Индивидуально",
            price: "от 100 000 ₽",
            features: ["B2B‑условия", "Склад/CRM", "Ритейл‑интеграции"],
          }].map((p) => (
            <div key={p.name} className="relative overflow-hidden rounded-2xl border border-neutral-900/10 bg-white/70 p-5 dark:border-white/10 dark:bg-white/[0.06]">
              <div className="absolute inset-x-0 -top-px h-1 bg-gradient-to-r from-transparent via-indigo-500/40 to-transparent" />
              ...
              <ul className="mt-3 space-y-1 text-sm text-neutral-700 dark:text-neutral-300">
                {p.features.map((f) => (
                  <li key={f} className="flex items-start gap-2"><span className="mt-1 inline-block h-1.5 w-1.5 rounded-full bg-indigo-500" />{f}</li>
                ))}
              </ul>
              <div className="mt-4">
                <Link href="/contacts" className="inline-flex h-9 items-center rounded-full bg-gradient-to-r from-indigo-600 to-blue-600 px-4 text-sm font-semibold text-white shadow-sm hover:from-indigo-700 hover:to-blue-700">Запросить предложение</Link>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="mt-10">
        <div className="relative overflow-hidden rounded-2xl border border-neutral-900/10 bg-gradient-to-r from-indigo-600 to-blue-600 p-5 text-white shadow-sm dark:border-white/10">
          <div className="text-base font-semibold tracking-tight">Запустим продажи онлайн</div>
          <p className="mt-1 text-sm opacity-90">Подключим оплату, наладим доставку и синхронизации — безболезненно.</p>
          <div className="mt-3 flex flex-wrap gap-2">
            <Link href="/contacts" className="inline-flex h-9 items-center rounded-full bg-white px-4 text-sm font-semibold text-indigo-700 shadow-sm hover:bg-white/90">
