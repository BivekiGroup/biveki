...
async function fetchCases() {
  const res = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL || ''}/api/graphql`, {
    ...
    body: JSON.stringify({ query: `query{ cases(service:account, limit: 2){ slug title summary } }` }), cache: 'no-store'
  });
...
const ADVANTAGES = [
  ...
  "Профили, уведомления, файлы",
  "Интеграции с CRM/1C и почтой",
  "Гибкие доступы по приглашениям",
];
    ...
    <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
      <section>
        ...
        <p className="mt-3 max-w-2xl text-sm leading-6 text-neutral-700 dark:text-neutral-300 sm:text-base">
          Закрытые разделы для клиентов и партнёров: быстрый вход, понятная навигация, прозрачные процессы.
        </p>
      </section>
      ...
      <section className="mt-8">
        ...
        <div className="mt-3 grid gap-3 lg:grid-cols-3">
          {[{
            name: "MVP кабинет",
            price: "от 70 000 ₽",
            features: ["Авторизация", "Профили", "Базовые роли"],
          }, {
            name: "Стандарт",
            price: "от 120 000 ₽",
            features: ["Роли и права", "Уведомления", "Интеграция с CRM"],
          }, {
            name: "Индивидуально",
            price: "от 180 000 ₽",
            features: ["Сложные сценарии", "SLA и мониторинг", "Миграции"],
          }].map((p) => (
            <div key={p.name} className="relative overflow-hidden rounded-2xl border border-neutral-900/10 bg-white/70 p-5 dark:border-white/10 dark:bg-white/[0.06]">
              <div className="absolute inset-x-0 -top-px h-1 bg-gradient-to-r from-transparent via-sky-500/40 to-transparent" />
              ...
              <ul className="mt-3 space-y-1 text-sm text-neutral-700 dark:text-neutral-300">
                {p.features.map((f) => (
                  <li key={f} className="flex items-start gap-2"><span className="mt-1 inline-block h-1.5 w-1.5 rounded-full bg-sky-500" />{f}</li>
                ))}
              </ul>
              <div className="mt-4">
                <Link href="/contacts" className="inline-flex h-9 items-center rounded-full bg-gradient-to-r from-sky-600 to-cyan-500 px-4 text-sm font-semibold text-white shadow-sm hover:from-sky-700 hover:to-cyan-600">Запросить предложение</Link>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="mt-10">
        <div className="relative overflow-hidden rounded-2xl border border-neutral-900/10 bg-gradient-to-r from-sky-600 to-cyan-500 p-5 text-white shadow-sm dark:border-white/10">
          <div className="text-base font-semibold tracking-tight">Построим кабинет под вашу задачу</div>
          <p className="mt-1 text-sm opacity-90">Обсудим роли, сценарии и интеграции. Предложим архитектуру и сроки.</p>
          <div className="mt-3 flex flex-wrap gap-2">
            <Link href="/contacts" className="inline-flex h-9 items-center rounded-full bg-white px-4 text-sm font-semibold text-sky-700 shadow-sm hover:bg-white/90">
