...
async function fetchCases() {
  const res = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL || ''}/api/graphql`, {
    ...
    body: JSON.stringify({ query: `query{ cases(service:apps, limit: 2){ slug title summary } }` }), cache: 'no-store'
  });
...
const ADVANTAGES = [
  "iOS/Android/desktop на единой базе",
  "Пуш‑уведомления и офлайн‑режим",
  "Авторизация, роли и интеграции",
  "Автообновления и телеметрия",
];
    ...
    <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
      <section>
        ...
        <p className="mt-3 max-w-2xl text-sm leading-6 text-neutral-700 dark:text-neutral-300 sm:text-base">
          Практичные приложения с офлайном и пушами: быстрое MVP → развитие по фидбеку.
        </p>
      </section>
      ...
      <section className="mt-8">
        ...
        <div className="mt-3 grid gap-3 lg:grid-cols-3">
          {[{
            name: "MVP",
            price: "от 150 000 ₽",
            features: ["Базовые экраны", "Авторизация", "Аналитика"],
          }, {
            name: "Продукт",
            price: "от 300 000 ₽",
            features: ["Пуши и офлайн", "Интеграции", "Релизы и QA"],
          }, {
            name: "Индивидуально",
            price: "от 450 000 ₽",
            features: ["Сложные фичи", "Техдизайн", "Поддержка"],
          }].map((p) => (
            <div key={p.name} className="relative overflow-hidden rounded-2xl border border-neutral-900/10 bg-white/70 p-5 dark:border-white/10 dark:bg-white/[0.06]">
              <div className="absolute inset-x-0 -top-px h-1 bg-gradient-to-r from-transparent via-rose-500/40 to-transparent" />
              ...
              <ul className="mt-3 space-y-1 text-sm text-neutral-700 dark:text-neutral-300">
                {p.features.map((f) => (
                  <li key={f} className="flex items-start gap-2"><span className="mt-1 inline-block h-1.5 w-1.5 rounded-full bg-rose-500" />{f}</li>
                ))}
              </ul>
              <div className="mt-4">
                <Link href="/contacts" className="inline-flex h-9 items-center rounded-full bg-gradient-to-r from-rose-600 to-pink-600 px-4 text-sm font-semibold text-white shadow-sm hover:from-rose-700 hover:to-pink-700">Запросить предложение</Link>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="mt-10">
        <div className="relative overflow-hidden rounded-2xl border border-neutral-900/10 bg-gradient-to-r from-rose-600 to-pink-600 p-5 text-white shadow-sm dark:border-white/10">
          <div className="text-base font-semibold tracking-tight">Сформируем MVP и план развития</div>
          <p className="mt-1 text-sm opacity-90">Определим ядро фич, спланируем релизы и аналитику.</p>
          <div className="mt-3 flex flex-wrap gap-2">
            <Link href="/contacts" className="inline-flex h-9 items-center rounded-full bg-white px-4 text-sm font-semibold text-rose-700 shadow-sm hover:bg-white/90">
